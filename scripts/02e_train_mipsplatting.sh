...
# [Step 02e] Mip-Splatting Training
# 3D Smoothing Filter 3DGS extension with Blackwell (RTX 50) optimization
...
# Ensure images directory/link exists for Mip-Splatting dataloader
...
MODEL_OUTPUT="${OUTPUT_DIR}/${SCENE_NAME}/mipsplatting"
mkdir -p "$MODEL_OUTPUT"

# 2. Training Execution
log_info "Starting Mip-Splatting Training (Scene: $SCENE_NAME)..."
...
if [ -n "$LATEST_CHKPNT" ]; then
    if [ -f "$SPARSE_PTS" ] && [ "$SPARSE_PTS" -nt "$LATEST_CHKPNT" ]; then
        ...
        log_info "Auto-resuming Mip-Splatting training from latest checkpoint: $(basename "$LATEST_CHKPNT")"
        EXTRA_TRAIN_ARGS+=("--start_checkpoint" "$LATEST_CHKPNT")
    fi
fi

python third_party/mip-splatting/train.py \
    -s "$TARGET_DATA_DIR" \
...
{
  "stage": "Stage 2e (Training)",
  "engine": "Mip-Splatting (3D Smoothing Filter 3DGS)",
...
log_ok "Mip-Splatting Training Completed! Results saved to $MODEL_OUTPUT"
